...
@Config(sdk=[28])
class CapabilityRetirementTest {
    private val app get() = RuntimeEnvironment.getApplication()
    private lateinit var store: ModuleStore
    private val id = "dev.construct.sky-watch"
    ...
    private fun fixture(folder: String): VerifiedPackage {
        ...
        val entry = Packages.catalog(File(dir, "index.json").readBytes()).single { it.id == id }
    ...
    // Reproduce alpha27's installed layout using the actual signed legacy package.
    ...
    private fun oldInstall(confirmed: Boolean = true): Installed {
        val f = fixture("sky-legacy-registry")
        ...
        File(app.filesDir, "module-state.json").writeText(JSONObject().put(id, JSONObject()
            ...
            .put("confirmed", confirmed).put("grants", JSONObject().put("sky.watch", true))).toString())
    ...
    @Test fun existingPackageIsManageableButCannotExecuteOrRegrant() {
        ...
        assertFalse("sky.watch" in old.granted)
        denied("MODULE_UPDATE_REQUIRED") { store.beginRun(old) }
        denied("MODULE_UPDATE_REQUIRED") { store.confirm(id) }
        denied("CAPABILITY_RETIRED") { store.setCapability(id, "sky.watch", true) }
        denied("CAPABILITY_RETIRED") { store.withCapability(old, "sky.watch") { fail() } }
        assertEquals(before, state()); retainedData()
        assertFalse("sky.watch" in Packages.supported)
    }
    @Test fun newLegacyInstallIsRejectedBeforeAnyStateOrCodeWrite() {
        denied("MODULE_UPDATE_REQUIRED") { store.install(fixture("sky-legacy-registry")) }
    ...
    @Test fun supportedUpdateRetainsDataAndDoesNotInheritLegacyAuthority() {
        ...
        assertFalse("net.http" in next.granted); assertFalse("location.read" in next.granted)
        store.beginRun(next); store.confirm(id); retainedData()
        denied("RUN_STALE") { store.withCapability(old, "sky.watch") { fail() } }
    }
    ...
    @Test fun retiredRollbackFailsAtomicallyAndCurrentVersionRemainsUsable() {
        oldInstall(); store.install(fixture("home-registry"), mapOf("net.http" to true))
        val before = state()
        denied("MODULE_UPDATE_REQUIRED") { store.rollback(id) }
        assertEquals(before, state()); retainedData()
        store.beginRun(store.installed().single())
        assertTrue(store.withCapability(store.installed().single(), "net.http") { true })
    }
    ...
    @Test fun catalogAllowsReplacingRetiredTrialButNotAnOrdinaryPendingTrial() {
        val release = CatalogVersion(id, "Sky Watch", "0.2.2", "new.zip", "a".repeat(64), "signature", false)
        assertTrue(CatalogInstallPresentation.choice(release, CatalogInstalledState("0.1.0", "b".repeat(64), false, true)).enabled)
        assertFalse(CatalogInstallPresentation.choice(release, CatalogInstalledState("0.1.0", "b".repeat(64), false)).enabled)
    }
}

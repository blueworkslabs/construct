...
@Config(sdk=[28])
class MeasureRetirementTest {
    private val app get() = RuntimeEnvironment.getApplication()
    private lateinit var store: ModuleStore
    private val id = "dev.construct.measure"
    ...
    // Reproduce alpha28's installed layout using the actual signed legacy package.
    ...
    private fun oldInstall(confirmed: Boolean = true): Installed {
        val f = fixture("measure-legacy-registry")
        ...
        File(app.filesDir, "module-state.json").writeText(JSONObject().put(id, JSONObject()
            ...
            .put("confirmed", confirmed).put("grants", JSONObject().put("photo.measure", true))).toString())
    ...
    private fun retainedData() = assertEquals(7, store.storage(id, JSONObject().put("op", "get").put("key", "counter")))
    @Test fun originalApi07RemainsReadableButEarlierApiCannotDeclareMeasure() {
        val legacy = fixture("measure-legacy-registry")
        assertEquals("0.7.0", legacy.manifest.api)
        val raw = JSONObject(legacy.files.getValue("manifest.json").toString(Charsets.UTF_8))
        raw.put("constructApi", JSONObject().put("min", "0.6.0").put("target", "0.6.0"))
    ...
    @Test fun existingPackageIsManageableButCannotExecuteOrRegrant() {
        ...
        assertFalse("photo.measure" in old.granted)
        denied("MODULE_UPDATE_REQUIRED") { store.beginRun(old) }
        denied("MODULE_UPDATE_REQUIRED") { store.confirm(id) }
        denied("CAPABILITY_RETIRED") { store.setCapability(id, "photo.measure", true) }
        denied("CAPABILITY_RETIRED") { store.withCapability(old, "photo.measure") { fail() } }
        assertEquals(before, state()); retainedData()
        assertFalse("photo.measure" in Packages.supported)
    }
    @Test fun newLegacyInstallIsRejectedBeforeAnyStateOrCodeWrite() {
        denied("MODULE_UPDATE_REQUIRED") { store.install(fixture("measure-legacy-registry")) }
    ...
    @Test fun supportedUpdateRetainsDataAndDoesNotInheritLegacyAuthority() {
        ...
        assertFalse("image.read" in next.granted); assertFalse("image.markers" in next.granted)
        store.beginRun(next); store.confirm(id); retainedData()
        denied("RUN_STALE") { store.withCapability(old, "photo.measure") { fail() } }
    }
    ...
    @Test fun catalogAllowsReplacingRetiredTrialButNotAnOrdinaryPendingTrial() {
        val release = CatalogVersion(id, "Pocket Measure", "0.2.0", "new.zip", "a".repeat(64), "signature", false)
        assertTrue(CatalogInstallPresentation.choice(release, CatalogInstalledState("0.1.3", "b".repeat(64), false, true)).enabled)
        assertFalse(CatalogInstallPresentation.choice(release, CatalogInstalledState("0.1.3", "b".repeat(64), false)).enabled)
    }
}

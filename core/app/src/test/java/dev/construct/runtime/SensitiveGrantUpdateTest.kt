...
class SensitiveGrantUpdateTest {
    ...
    private val sensitive = setOf("device.tone", "contacts.read", "camera.photo", "photos.library", "image.analyze", "image.read", "image.markers", "location.read", "net.http")
    ...
    @Test fun removedSensitiveCapabilitiesDefaultOffWhenReintroduced() {
        store.install(fixture("3.1.0"), sensitive.associateWith { true }); store.confirm(id)
        store.storage(id, JSONObject().put("op", "set").put("key", "counter").put("value", 7))
        store.install(fixture("3.2.0")); store.confirm(id); store = ModuleStore(app)
        store.install(fixture("3.3.0")); store = ModuleStore(app)
        assertDenied()
    ...
    @Test fun unchangedDeclarationsAndExplicitReapprovalRetainAuthority() {
        store.install(fixture("3.1.0"), sensitive.associateWith { true }); store.confirm(id)
        store.install(fixture("3.3.0")); store.confirm(id)
        assertTrue(store.installed().single().granted.containsAll(sensitive))
        store.install(fixture("3.2.0")); store.confirm(id)
        store.install(fixture("3.1.0"), sensitive.associateWith { true }); store = ModuleStore(app)
        assertTrue(store.installed().single().granted.containsAll(sensitive))
    }
    @Test fun staleSensitiveGrantEntriesCannotOverrideDefaultOffInstallChoices() {
        ...
        store.install(fixture("3.3.0"))
        assertDenied()
    }
    @Test fun reintroducedLegacyStorageDoesNotUndoExplicitRevocation() {
        store.install(fixture("3.1.0")); store.setCapability(id, "storage.kv", false); store.confirm(id)
        store.install(fixture("3.4.0")); store.confirm(id)
        store.install(fixture("3.3.0")); store = ModuleStore(app)
        val module = store.installed().single()

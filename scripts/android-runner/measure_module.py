...
receipt = {'scope':'Module-owned Measure prototype, not final pilot acceptance','apkSha256':a.sha,'checks':[],'complete':False,'stopped':False}
started = False
...
try:
    ...
    events=diagnostics()
    if not any(e.get('code')=='INSTALLED_TRIAL' and e.get('packageDigest')==a.module_sha for e in events):raise RuntimeError('Wrong signed preview')

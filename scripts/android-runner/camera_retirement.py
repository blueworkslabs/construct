...
run = CONFIG.root / 'results' / (datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ') + '-camera-retirement-' + uuid.uuid4().hex[:8])
run.mkdir(parents=True)
os.environ['CONSTRUCT_RESULTS'] = str(run)
import ui
from ui import adb, nodes, labels, tap, tap_node, find, capture as ui_capture
...
URL = a.catalog
receipt = {'scope': 'Native Camera retirement across exact alpha30-to-alpha31 upgrade',
...
def reach(label):
    ...
    previous=None
    for i in range(24):
        ns=nodes();w,h=ui._device.window_size()
        for n in ns:
            if label not in (n.get('text'),n.get('content-desc')):continue
            b=list(map(int,re.findall(r'-?\d+',n.get('bounds',''))))
            if len(b)==4 and 0<=b[1]<b[3]<=h and b[2]>b[0]:
                if previous==n.get('bounds'):return n
                previous=n.get('bounds');time.sleep(.3);break
        else:
            start,end=(.85,.4) if i<12 else (.4,.85)
            adb('shell','input','swipe',str(w//2),str(int(h*start)),str(w//2),str(int(h*end)),'250')
        time.sleep(.2)
...
def consent_switch(label):
    ...
    if len(matches)!=1:raise RuntimeError('Missing trusted grant switch')
    return matches[0]

def capture(name):
    ui_capture(name);target=run/(name+'-display');target.mkdir()
...
def private_hashes():
    ...
    assert adb('shell','getprop','ro.kernel.qemu').strip()=='1' and adb('shell','getprop','ro.build.type').strip()=='userdebug'
    adb('root');adb('wait-for-device')
    try:
        assert adb('shell','id','-u').strip()=='0'
        folder='/data/user/0/dev.construct.runtime/files/camera-photos/dev.construct.camera/'
        names=adb('shell','ls','-A',folder).split()
        assert names and all(re.fullmatch(r'[a-f0-9-]{36}\.jpg',n) for n in names)
        return {n:hashlib.sha256(adb('exec-out','cat',folder+n,binary=True)).hexdigest() for n in names}
    finally:restore_shell_identity(adb)
...
try:
    ...
    (CONFIG.root/'camera-emulated.flag').write_text('emulated\n')
    avd, snapshot, _ = CONFIG.profile(True)
    ...
    for option,value in [('-avd',avd),('-camera-back','emulated'),('-camera-front',CONFIG.front_camera(True))]:
        if option not in command or command[command.index(option)+1]!=value:raise RuntimeError('Synthetic camera not selected')
    ...
    library();select_after('Pocket Camera · 0.1.4',('Module access',))
    tap_node(reach('Allow Android camera access'));find('While using the app');tap('While using the app')
    tap_node(reach('Reopen module'));find('Open camera workspace');tap('Open camera workspace')
    from ui import camera_controls_top
    deadline=time.monotonic()+60
    while time.monotonic()<deadline:
        camera_controls_top()
        if any(t.startswith('Camera preview ready.') for t in labels()):break
        time.sleep(.3)
    else:raise RuntimeError('Old native preview failed')
    tap('Take photo');find('Saved photos: 1 / 8');find('Saved photo preview')
    capture('old-native-workspace');tap('Close camera');library()
    original=private_hashes();receipt['originalPhotoHashes']=original
    if len(original)!=1:raise RuntimeError('Expected one real synthetic shutter photo')
    ...
    adb('install','-r',str(a.new.resolve()),timeout=120); verify_installed(a.new_sha); assert private_hashes()==original,'Upgrade changed private originals'; launch()
    ...
    library(); _stable_card('Pocket Camera · 0.1.4', ('Find update',))
    ...
    switches=[n for n in nodes() if n.get('checkable')=='true' and n.get('content-desc')=='camera.capture']
    ...
    capture('fresh-image-consent'); tap_node(reach('Allow & install')); installed_status()
    events=diagnostics()
    if not any(e.get('code')=='INSTALLED_TRIAL' and e.get('packageDigest')==candidates['camera']['sha256'] for e in events):
        raise RuntimeError('Modern signed module digest not verified')
    library(); select_after('Pocket Camera · '+candidates['camera']['version'], ('Open',)); find('Take a photo')
    ...
    done('Unconfirmed retired module updates to exact modern signed module and displays retained originals only after new pixel/library consent')
    select_after('Pocket Camera · '+candidates['camera']['version'], ('Roll back',)); tap('Restore'); library()
    ...
    select_after('Pocket Camera · '+candidates['camera']['version'], ('Open',)); find('Take a photo')
    ...
    select_after('Pocket Camera · 0.1.4', ('Review & install',)); settled_host_surface(); scroll_top(); find(error)
    ...
    assert private_hashes()==original,'Original photo changed during migration or rejected rollback'
    done('Real native shutter original remains byte-identical after migration and rejected rollback')
    ...
    (CONFIG.root/'camera-emulated.flag').unlink(missing_ok=True)
    save()
